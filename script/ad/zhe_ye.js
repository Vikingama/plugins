...
function unlockBlockedKeywords() {
  ...
  try {
    ...
    if ($.request.method === 'GET') {
      ...
      if ($.env.isQuanX) {
        o = { body: r, headers: t, status: 'HTTP/1.1 200 OK' };
      } else {
        o = { response: { body: r, headers: t, status: 200 } };
      }
      ...
      if (!!$.request.body) {
        ...
        if (i === false) {
          ...
          if ($.env.isQuanX) {
            o = { body: e, headers: t, status: 'HTTP/1.1 200 OK' };
          } else {
            o = { response: { body: e, headers: t, status: 200 } };
          }
          ...
          if ($.env.isQuanX) {
            o = { body: e, headers: t, status: 'HTTP/1.1 400 Bad Request' };
          } else {
            o = { response: { body: e, headers: t, status: 400 } };
          }
        }
      }
      ...
      if ($.env.isQuanX) {
        o = { body: n, headers: r, status: 'HTTP/1.1 200 OK' };
      } else {
        o = { response: { body: n, headers: r, status: 200 } };
      }
...
function processUserInfo() {
  ...
  try {
    ...
    if (
      ...
      $.logger.debug(
        `当前用户 id：${e['id']}，是否为 VIP：${e['vip_info']['is_vip']}`
      );
...
function manageBlackUser() {
  ...
  $.logger.debug(`当前用户 id：${r.id}，脚本黑名单：${JSON.stringify(n)}`);
  if ($.request.method === 'GET') {
    try {
    ...
    try {
      ...
      if (e.hasOwnProperty('name') && e.hasOwnProperty('id')) {
        $.logger.debug(
          `当前需要加入黑名单的用户 Id：${e['id']}，用户名：${e['name']}`
        );
        if (e['id']) {
          n[e['name']] = e['id'];
          $.data.write(blockedUsersKey, n, r.id);
          ...
          $.logger.error(`${e['name']}写入脚本黑名单失败，没有获取到用户 Id。`);
    ...
    try {
      ...
      if (e.success) {
        ...
        if (t) {
          $.logger.debug(`当前需要移出黑名单的用户 Id：${t}`);
          for (let e in n) {
            if (n[e] === t) {
              delete n[e];
              $.data.write(blockedUsersKey, n, r.id);
...
function autoInsertBlackList() {
  ...
  try {
    ...
    if (t.name && t.id && t['is_blocking'] === true) {
      ...
      if (!e[t.name]) {
        $.logger.debug(
          `当前需要加入黑名单的用户 Id：${t['id']}，用户名：${t['name']}`
        );
        e[t['name']] = t['id'];
...
function removeMoments() {
  ...
  try {
    ...
    const n = getUserInfo();
    let o = $.data.read(blockedUsersKey, '', n.id);
    o = !!o ? o : {};
    let t;
    ...
    t = e.data.filter(e => {
      const t =
        ...
        typeof o[e.target['origin_pin'].author.name] != 'undefined';
      const r = s && e['target_type'] === 'zvideo';
      const n = a && e.type === 'recommend_user_card_list';
      const i = l && e.type === 'message_activity_card';
      return !(t || r || n || i);
    });
    e['data'] = t;
...
async function _checkPaidContentByLocal(n, i) {
...
async function removeRecommend() {
  ...
  try {
    ...
    const i = $.data.read('zhihu_settings_request_content', 'local');
    const o = getUserInfo();
    let u = $.data.read(keywordBlockKey, '', o.id);
    u = m && !!u ? u : [];
    let f = $.data.read(blockedUsersKey, '', o.id);
    f = y && !!f ? f : {};
    const s = i => {
      const e = JSON.stringify(i);
      const t =
        i['card_type'] === 'slot_event_card' ||
        i['card_type'] === 'slot_video_event_card' ||
        i.hasOwnProperty('ad') ||
        (i['brief'] && i['brief'].indexOf('slot_card') >= 0) ||
        (i['extra'] && i['extra']['type'] === 'Training');
      const r =
        t !== true &&
        e.search(/"(type|style)+"\s?:\s?"(drama|zvideo|Video|BIG_IMAGE)+"/i) >=
          0;
      const n = r && p;
      const o = r !== true && e.search(/"(type|style)+"\s?:\s?"pin"/i) >= 0;
      const s = o && g;
      ...
      if (r !== true && m) {
        for (let n = 0; n < u.length; n++) {
          if (e.search(u[n]) >= 0) {
            if ($.isDebug) {
              let e = i['common_card']['feed_content']['title']['panel_text'];
              let t = i['common_card']['feed_content']['content']['panel_text'];
              let r = '';
              try {
                r =
                  i['common_card']['feed_content']['title']['action'][
                    'intent_url'
                  ];
              } catch {}
              $.logger.debug(
      ...
      try {
        d =
          ...
          y &&
          f &&
          i['common_card']['feed_content']['source_line']['elements'][1][
            'text'
          ]['panel_text'] in f;
      } catch {
        d = false;
    ...
    if (r === true) {
      ...
      if (r.length > 0 && i === 'cloud') {
        await _checkPaidContentByCloud(r, n);
      } else if (r.length > 0 && i === 'local') {
...
function removeQuestions() {
  ...
  try {
    ...
    n = !!n ? n : {};
    let e = JSON.parse($.response.body);
    const o = $.data.read('zhihu_settings_blocked_users', false);
    $.logger.debug(`当前黑名单列表：${JSON.stringify(n)}`);
    let i = $.data.read(blackAnswersIdKey, []);
    delete e['ad_info'];
    if (e['data']) {
      let r = [];
      for (let t of e.data) {
        ...
        if (!c) {
          r.push(t);
        } else if (c === true && i.includes(a) === false) {
          i.push(a);
          $.notification.debug(`记录黑名单用户${e}的回答 Id:${a}`);
        }
      }
      e.data = r;
    }
    $.data.write(blackAnswersIdKey, i);
...
function modifyAnswer() {
  ...
  try {
    ...
    if (t !== '') {
      ...
      const i = e.lastIndexOf(n) + n.length;
      r = { body: e.slice(0, i) + t + e.slice(i) };
    }
  } catch (e) {
      ...
      if ($.data.read('zhihu_settings_blocked_users', false) === true) {
        ...
        if (typeof t.data != 'undefined') {
          t.data.forEach(e => {
            ...
            const n = r !== '';
            const i = typeof s[t] != 'undefined';
            const o = typeof s[r] != 'undefined';
            if (i === true || o === true) {
              if (i && !n && $.request.url.indexOf('root_comment') > 0) {
              ...
              } else if (i && o && $.request.url.indexOf('child_comment') > 0) {
                $.notification.debug(
...
function modifyAnswersNextData() {
  ...
  try {
    if (!!$.response.body) {
      let e = JSON.parse($.response.body);
      let t = getUserInfo();
      let n = $.data.read(blockedUsersKey, {}, t.id);
      $.logger.debug(`脚本黑名单用户：\n${JSON.stringify(n)}`);
      let i = [];
      e.data.data.forEach(e => {
        const t =
          e['resource_type'] === 'ad' ||
          e['extra']['recommend_start'] === true ||
          e['next_guide']['change_type'] === 1 ||
          e['next_guide']['title'] === '推荐的相关内容';
        if (t === true) {
          return;
        }
        e['ad_info'] = { data: '' };
        const r = typeof n[e.data.author.name] != 'undefined';
        $.logger.debug(`用户${e.data.author.name}是否在黑名单中：${r}`);
        if (
...
function MagicData(s, a) {
  ...
  const f = (e, t = null, r = '', n = false, i = null) => {
    let o = i || l.data;
  ...
  const g = (e, t = null, r = '', n = false, i = null) => {
    let o = '';
    if (i || s.isNode) {
      o = f(e, t, r, n, i);
    } else {
  ...
  const e = (t, r, n, i = c, o = null) => {
    r = d(r);
    const e = g(t, null, n, false, o);
    if (i(e, r) === true) {
      return false;
    } else {
      const s = h(t, r, n, o);
      let e = g(t, null, n, false, o);
      if (i === c && typeof e === 'object') {
        return s;
      }
      return i(r, e);
    }
  ...
  const t = (e, t = '', r = null) => {
    ...
    if (r || s.isNode) {
      ...
      if (!t) {
        ...
        n = o(n);
        delete n[t];
        const i = JSON.stringify(n, null, 4);
        h(e, i);
      }
    }
...
function MagicHttp(c, d) {
  ...
  const o = { request: new e(), response: new e(false) };
  let p = [];
  let h = [];
  let y = true;
  function $(e) {
      ...
      if (c.isSurgeLike) {
        i = o => {
          return new Promise((n, i) => {
            $httpClient[e.toLowerCase()](o, (t, r, e) => {
              if (t) {
                let e = {
                  name: t.name || t,
                  message: t.message || t,
