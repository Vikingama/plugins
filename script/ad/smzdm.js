...
(() => {
  ...
  if (magicJS.isResponse) {
    switch (true) {
      case /^https?:\/\/app-api\.smzdm\.com\/util\/loading/.test(
        magicJS.request.url
      ):
        try {
          ...
          magicJS.logError(`去除 APP 启动广告出现异常：${err}`);
        }
        break;
      case /^https?:\/\/homepage-api\.smzdm\.com\/v3\/home/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
          ...
          obj.data.component.forEach(element => {
            ...
            if (
              element.zz_type !== 'top_banner' &&
              element.zz_type !== 'hongbao'
            ) {
        ...
        break;
      case /^https?:\/\/haojia-api\.smzdm\.com\/home\/list/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
        ...
        break;
      case /^https?:\/\/haojia\.m\.smzdm\.com\/detail_modul\/article_releated_modul/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
        ...
        break;
      case /^https?:\/\/baike-api\.smzdm\.com\/home_v3\/list/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
          obj.data.rows = obj.data.rows.filter(element => {
            return (
              !element.hasOwnProperty('ad_banner_id') ||
              element.ad_banner_id == ''
            );
          });
          response = { body: JSON.stringify(obj) };
        } catch (err) {
        ...
        break;
      case /^https?:\/\/s-api\.smzdm\.com\/sou\/filter\/tags\/hot_tags/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
        ...
        break;
      case /^https?:\/\/s-api\.smzdm\.com\/sou\/list_v10/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
        ...
        break;
      case /^https?:\/\/zhiyou\.m\.smzdm\.com\/user\/vip\/ajax_get_banner/.test(
        magicJS.request.url
      ):
        try {
          let obj = JSON.parse(magicJS.response.body);
          ...
          magicJS.logError(`值会员权益中心 banner 去广告出现异常：${err}`);
        }
        break;
      default:
